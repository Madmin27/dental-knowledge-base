...
import {watchDraft,guardDraft} from './drafts.js';
...
export async function installContributions({captureView=()=>({kind:'technical',version:1,page:location.pathname==='/tooth-interior'?'interior':location.pathname==='/report'?'report':'anatomy',structure:'viewer'}),restoreView,catalog={structures:[{name:'viewer',label:t('Teknik bildirim')}]}}={}){
  if($('#contribution-dialog'))return;
  ...
  let pending;let activeForm=false;let releaseReceipt;
  guardDraft(()=>Boolean(pending));
  button.onclick=()=>{
    if(pending||activeForm){dialog.showModal();return;}
    content.replaceChildren();const view=captureView();
    const form=node('form',undefined,content);form.id='contribution-form';activeForm=true;const draft=watchDraft(form);
    ...
    form.onsubmit=async event=>{
      ...
      try{
        const r=await request('',pending.key,{id:pending.id,...pending.body});const key=pending.key;pending=null;draft.clear();activeForm=false;content.replaceChildren();node('h3',t('Bildiriminiz kaydedildi'),content);node('p',t('Takip no: ')+r.id+' · '+labels[r.status],content);receipt(content,r.id,key);activeForm=true;releaseReceipt=guardDraft(()=>true);const saved=node('button',t('Takip bağlantısını kaydettim'),content);saved.type='button';saved.id='receipt-saved';saved.onclick=()=>{releaseReceipt?.();releaseReceipt=null;saved.disabled=true;};
        const again=node('button',t('Yeni katkı'),content);again.onclick=()=>{if(releaseReceipt){alert(t('Önce takip bağlantısını kaydedin.'));return;}activeForm=false;button.click();};
      ...
      finally{for(const field of form.elements)field.disabled=Boolean(pending)&&field!==submit&&!field.readOnly;submit.disabled=false;}
    };
    dialog.showModal();
  };
  // Restore only an explicitly requested, authenticated record, never arbitrary URL coordinates.
  const params=new URLSearchParams(location.hash.slice(1));
  if(restoreView&&params.get('view')&&params.get('key')){
...
async function trackingPage(){
  ...
  let noteDraft='',refsDraft='',pendingEvent=null;
  ...
  async function load(){
    try{const r=await request('/'+id,key);body.replaceChildren();message.textContent='';node('h2',labels[r.status],body);node('p',t('Alınma: ')+new Date(r.createdAt).toLocaleString(dateLocale)+t(' · Sürüm: ')+r.revision,body);
      ...
      const discard=node('button',t('Taslağı sil'),form);discard.type='button';discard.onclick=()=>{if(pendingEvent){alert(t('Önce belirsiz gönderimi yeniden deneyin.'));return;}if(confirm(t('Bu taslağı silmek istediğinizden emin misiniz?'))){noteDraft='';refsDraft='';note.value='';refs.value='';}};
      const send=node('button',t('Açıklamayı ekle'),form);send.type='submit';form.onsubmit=async e=>{e.preventDefault();send.disabled=true;note.disabled=true;refs.disabled=true;discard.disabled=true;try{if(!pendingEvent){noteDraft=note.value;refsDraft=refs.value;pendingEvent={eventId:hex(16),revision:r.revision,note:note.value,evidence:links(refs.value)};}await request('/'+id+'/events',key,pendingEvent);pendingEvent=null;noteDraft='';refsDraft='';await load();}catch(e){if(e.status&&e.status<500&&e.status!==429){pendingEvent=null;if(e.status===409)await load();}message.textContent=t(e.message);}finally{send.disabled=false;note.disabled=Boolean(pendingEvent);refs.disabled=Boolean(pendingEvent);discard.disabled=Boolean(pendingEvent);}};
...
trackingPage();
if($('#technical-report'))installContributions();
