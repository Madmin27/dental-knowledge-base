...
await click('#receipt-saved');await call('Page.navigate',{url:tracking});await wait('document.querySelector("#replay-view")');assert.equal(await ev('document.querySelectorAll("#contribution-tracking img").length'),0);assert.ok(await ev('document.body.textContent.includes("<img src=x")'));passed.push('fresh page tracks privately; hostile text remains text');
await ev(`document.querySelector('[name=note]').value='Synthetic additional evidence';document.querySelector('#contribution-tracking form').requestSubmit()`);await wait(`document.body.textContent.includes(${JSON.stringify(turkish?'Sürüm: 1':'Revision: 1')})`);passed.push('contributor adds evidence with retained history');
...
await call('Page.reload');await wait(`document.body.textContent.includes(${JSON.stringify(turkish?'Ön incelemede':'In triage')})`);await shot('contribution-tracking');passed.push('authenticated maintainer status and timeline visible to contributor');
